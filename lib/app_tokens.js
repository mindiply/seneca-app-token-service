...
module.exports = function () {
    ...
    var redis_client = options.redis_client || require('../redis/redis_client')();
    var log = options.log || require('../conf/logging');
    var expiration_seconds = options.expiration_seconds || APP_TOKEN_EXPIRATION_SECONDS;

    return {

        createAppToken: function createAppToken(user_id, scopes) {

            var p = new Promise(function (resolve, reject) {
                try {
                    (function () {
                        var token = crypto.randomBytes(256).toString('base64');
                        var token_key = _token2UserScopeKey(token);
                        if (!token_key) throw new Error('Incorrect token key');

                        var token_data = {
                            token: token,
                            expires_in_s: expiration_seconds
                        };
                        resolve(Promise.all([redis_client.lpush.apply(redis_client, [token_key, user_id].concat(_toConsumableArray(scopes))), redis_client.expire(token_key, expiration_seconds)]).then(function (results) {
                            return token_data;
                        }));
                    })();
                } catch (err) {
        ...
        userScopesByToken: function userScopesByToken(token) {
            var p = new Promise(function (resolve, reject) {
                try {
                    ...
                    var q = redis_client.lrange(token_key, 0, -1).then(function (values) {
